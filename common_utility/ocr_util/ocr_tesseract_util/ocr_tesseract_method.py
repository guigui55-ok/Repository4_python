...
def get_tools_by_initialize_tresseract() -> pyocr.TOOLS:
...
class OcrBox:
    ...
    # 縦読みか横読みか
    direction = ConstOcr.DIRECTION_HORIZON
    # 次の距離と1文字当たりの大きさの比率
    ...
    def __init__(
        ...
        direction_:direction = ConstOcr.DIRECTION_HORIZON,
        ...
        if not flag: raise Exception('set_ocr_result Failed -> return')
        if next_ocr_box != None:
            flag = self.calc_next_distance(next_ocr_box)
            if not flag: raise Exception('calc_next_distance Failed -> return')

    def set_ocr_result(self,ocr_result:pyocr.builders.Box,direction_:direction):
        ...
        # 1文字当たりの幅
        if direction_ == ConstOcr.DIRECTION_HORIZON:
            self.char_point.x = self.width / len(self.str_value)
            self.char_point.y = self.height / 1
        
        if direction_ == ConstOcr.DIRECTION_VERTICAL:
...
class OcrBoxes():
    ...
    def __init__(
        ...
        threshold_for_judging_separation = ConstOcr.DEFAULT_THRESHOLD,
        ...
        try:
            ...
            for result in ocr_result_list:
                if ocr_direction_is_horizon:
                    direc = ConstOcr.DIRECTION_HORIZON
                else:
                    direc = ConstOcr.DIRECTION_VERTICAL
...
def box_is_far_apart(
    ...
    separation_border_value:float=ConstOcr.DEFAULT_THRESHOLD)->bool:
    try:
