...
def reprace_newline(value:str)->str:
    ret = value.replace('\n','')
    return ret
...
def get_result_subprocess_run_command(command,is_logout=False):
...
def save_file_from_android(
    ...
    is_logout_stdout=True)->bool:
        ...
        flag , ret = excute_command_adb(cmd, device_id, is_logout_stdout)
        return flag
...
def adb_reboot(
    ...
    """Android デバイスを再起動させる"""
    cmd = 'adb reboot'
    flag , result = excute_adb_command(cmd,device_id,is_logout_stdout,is_not_adb_shell=True)
    return flag
...
def get_center_from_rect(point_rect:'list[int,int,int,int]'):
    """int か str 型の(left_top_x,left_top_x,right_bottom_x,right_bottom_y)tuple から
        中央の値を取得する
    """
    try:
        if(len(point_rect) < 4):
            log_error('argument length < 4')
            ...
            log_error('argument length case else -> unexpected case')
            return (0,0)
        return (0,0)
    except Exception as e:
        log_error(e)
...
def touch_screen(
    ...
    if times > 1: log_error('touch_screen multiple time : Unimplemented')
...
def is_connect_android(device_id = '' ,is_logout_stdout:bool=True) -> bool:
    ...
    if len(dev_list) < 0:
        log_error('get_connect_adb_devices : len(dev_list) < 0 , False')
        return False
    # デバイス指定なしでつながっている場合は
    if device_id == '':
        return True
...
def get_android_version(device_id:str='', is_logout_stdout:bool=True):
    """Androidのバージョンを取得する"""
    cmd = 'adb shell getprop ro.build.version.release'
    result:str = ''
    flag, result = excute_command_adb_shell(cmd,device_id,is_logout_stdout)
    if flag:
        return reprace_newline(result)
    else:
        return ''

def get_android_build_version(device_id:str='', is_logout_stdout:bool=True):
...
def cnv_complete_process(
    result,
    func_name='',
    is_logout_stdout=False,
    ret_type=2):
    """CommandPrompt(Terminal)で得られた結果を文字列へ変換する
    ret_type=1:bool, ret_type=2:str"""
    try:
        # 結果をboolで取得する
        ...
        if ret_type == 1:
            ...
            log_error('cnv_complete_process : ret_type is invalid. return None')
            return None
    except Exception as e:
        log_error(e)
        # 引数によって戻り値の型を変更する
...
def get_package_version(package_name):
    ...
    if is_success_adb_result(result,'get_package_version'):
        ...
        log_error('get_package_version Failed')
...
def get_servece_call_iphonesubinfo(number,is_logout=True):
    ...
    if level.isnumeric():
        ...
        log_error('get_servece_call_iphonesubinfo Failed:get api level Failed')
        log_error('number = '+ str(number))
        log_error('command result = '+level)
        return ''
