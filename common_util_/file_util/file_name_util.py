...
def add_str_to_file_name_by_list(
    ...
    if len(add_str_list) < 1 :
        logger.exp.error(mn + 'len(add_str_list) < 1 , return')
        return base_file_name

    for val in add_str_list:
        ret = add_str_to_file_name(logger,base_file_name,val,delimita,position)
    return ret
...
def add_str_to_file_name(
    ...
    if base_file_name == '': 
        logger.exp.error(mn + 'base_file_name is blank , return')
        return base_file_name
