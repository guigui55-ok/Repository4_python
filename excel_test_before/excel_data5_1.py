...
def main():
    ...
    ###########
    # cell_values_list = get_values_from_range_address(ex_data.book, ex_data.sheet, range_address)
    # cell_values_list = get_values_from_range_address_np(ex_data.book, ex_data.sheet, range_address)
    cell_values_list = ex_data.get_values_from_range_address_np()
    # print('cell_values_list = ')
    # import pprint
    # pprint.pprint(cell_values_list)
...
"""
https://gammasoft.jp/blog/openpyxl-get-data-containing-range/

帳票のように書式設定で体裁を整えているファイルでは、余分は部分までデータ範囲として処理してしまうことがある
スペースや値が空でも書式設定あればデータ範囲となる
書式：罫線、中央ぞろえなど

Worksheet.iter_rows()を使ってCellを順番に取得する
# https://www.relief.jp/docs/openpyxl-iter-rows-iter-cols-method.html
