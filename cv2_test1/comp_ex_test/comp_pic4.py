...
def main():
    ...
    target_path = r'F:\ZPICTURE\jpg ss new\jpg New\0jpg op base image\op new__________new2'
    sort_dir = r'F:\ZPICTURE\jpg ss new\jpg New\0jpg op base image double'
    ...
    jpg_path_list = get_file_path_list_new(target_path,'.jpg')
    logger.info('list_count = {}'.format(len(jpg_path_list)))
    #excute
...
def save_all_pics(path_list:'list[str]',sort_dir:str,logger_):
    ...
    for base_path in path_list:
        ...
        if count % 200 == 0:
            logger.info('reading count[{} / {}]'.format(count,len(path_list)))
...
def get_file_path_list_new(root_path:str,endswith_str:str):
    import common_utility.file_util.file_list as file_list_util
    path_list = file_list_util.get_file_path_list(root_path)
    path_list_all = file_list_util.get_file_path_list_all_up_to_bottom_dir(path_list,'/*')
    target_list = [x for x in path_list_all if x.endswith(endswith_str)]
    return target_list
