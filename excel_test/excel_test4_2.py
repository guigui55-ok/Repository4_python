...
def main():
    ...
    # キャプション ■TableA の下にある矩形のデータ範囲を取得する
    cellu = CellUtil(sheet)
    begin_cell  = cellu.set_address_by_find('■TableA','A1', 'F38')
    begin_address = begin_cell.coordinate

    # ■TableB
    cellu.address = 'U10'
    cellu.set_cell_by_a1('U10')
    print('get_value')
    print(cellu.get_value())
    print('begin_address = {}'.format(begin_address))
    begin_cell_b = cellu.move_address(1,0)
    ...
    range_cells = cellu.get_range_cells()
    table_address = cellu.get_range_address_in_cell_list(range_cells)
    print('table_address2 = {}'.format(table_address))
    ###
    print('=========')
    address = 'A1:B5'
