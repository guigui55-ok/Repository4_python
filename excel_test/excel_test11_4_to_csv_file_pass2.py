...
# _READ_DIR = r''
_FILE_PATH_LIST = [
    Path(__file__).parent.joinpath('excel_with_pass.xlsx'),
    Path(__file__).parent.joinpath('excel_with_pass2.xlsx'),
]
...
def main(file_path):
    ...
    read_dir_path = Path(file_path).parent
    if _SAVE_DIR!='':
        save_dir_path = Path(_SAVE_DIR)
    else:
    ...
    password = 'abc'
    unlock_file_name = Path(file_name).stem + '_unlock' + Path(file_name).suffix
    ...
    csv_file_name = Path(file_path).stem + '.csv'
...
if __name__ == '__main__':
    for i, file_path in enumerate(_FILE_PATH_LIST):
        print('#####')
