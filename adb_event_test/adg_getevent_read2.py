...
class const(enum.Enum):
    FILE_NAME_BIN = 'sendevent.bin'
    FILE_NAME_GETEVENT = 'getevent.txt'
    ANDROID_DIR = '/sdcard/'
...
def read_event_file():
    try:
        ...
        file_name = const.FILE_NAME_GETEVENT.value
        ...
        bin_name = const.FILE_NAME_BIN.value
        ...
        android_path = '/sdcard/'
        android_path = const.ANDROID_DIR.value
...
class AdbEventReader():
    read_lines = None

    def read_byetes(self,file_path):
        f = open(file_path, 'rb')
        self.read_lines = f.readlines()
        f.close()


def read_bin_from_android():
    try:
        # set log object
        logger = initialize_logger()
        # set path
        ...
        file_name = 'getevent.txt'
        file_name = const.FILE_NAME_GETEVENT.value
        save_file_name = 'save_getevent.bin'
        file_path = os.path.join(dir_path,save_file_name)
        # set file name
        get_file_name = const.FILE_NAME_BIN.value
        android_path = const.ANDROID_DIR.value
        flag = adb_common.save_file_to_pc_from_android(logger,
            dir_path,android_path,get_file_name)
        if not flag:
            logger.exp.error('save_file_to_pc_from_android failed , return')
            return
        
        read_path = os.path.join(dir_path,get_file_name)
        reader = AdbEventReader()
        reader.read_byetes(read_path)

    except:
        traceback.print_exc()

def main():
    read_bin_from_android()
    # read_event_file()
