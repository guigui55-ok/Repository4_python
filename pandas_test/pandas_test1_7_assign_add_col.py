...
df_d = df_c.groupby('身長基準').get_group('high_')
# print(df_d)
...
# 体重列を追加し、ランダムな整数値を設定（50kgから100kg）
df['体重'] = np.random.randint(50, 101, size=len(df))

# 身長をメートルに変換
df['身長m'] = df['身長'] / 100

# BMI列を追加し、BMIを計算
df['BMI'] = df['体重'] / (df['身長m'] ** 2)
