...
class JsonDict():
    key:str=None
    value=None
    dictlist_elements:'list[JsonDict]'=None
    ...
    class PrintJsonState():      
        class JsonStateBlock():
            ...
            def get_edge(self,is_begin:bool,value_type:int)->str:
                if value_type == JsonDict.ValueTypeConst.DICT:
                    if is_begin:
                        return '{'
                    else:
                        return '}'
                if value_type == JsonDict.ValueTypeConst.LIST:
                    if is_begin:
                        return '['
                    else:
                        return ']'
        ...
        def end_block(self):
            ...
            if len(self.blocks)>1:
                block:JsonDict.PrintJsonState.JsonStateBlock = self.blocks[-1]
            if len(self.blocks)>1:
                self.blocks = self.blocks[:-1]
            if block == None:
                print('[JsonDict]Alert:JsonStateBlock.end_block():block == None')
                return ''
    ...
    def get_json_str(self,now_value:str='',now_state:PrintJsonState=None,):
        ...
        for i in range(len(self.dictlist_elements)):
            ...
            if el.is_terminate_dict:
                ...
                print('***** JsonDict.get_json_str,el.value_type:else')
    ...
    def get_key_and_value_if_terminate_dict(self):
        if self.is_terminate_dict:
            if isinstance(self.value , dict):
                ...
                print('***** JsonDict.get_key_and_value_if_terminate_dict: not dict')
        else:
            return '',''
    ################################################################################
            ...
            if self.__is_terminate:
                if isinstance(self.value,dict):
                    ...
                    for k in keys:
                        key = str(k)
                        break
            elif self.parent_key == KEY_NAME_LIST:
                keys = self.value.keys()
                for k in keys:
                    key = str(k)
                    break
    ...
    def set_value(self,value,hierarchy:int,key,parent_key='',value_type=0):
        ...
        if isinstance(value,dict):
            ...
            for child_key in value.keys():
                ...
                dl = JsonDict(target_value, self.hierarchy + 1, child_key , self.key, 0)
                # dl.key = key
    ...
    def update_part(self,value,hierarchy=0):
        ...
        ** そこまでたどってから、valueを代入する
        ...
        if not isinstance(value,JsonDict):
            update_val:JsonDict = JsonDict(value,hierarchy,'',)
        else:
            update_val = value
                    ...
                    if dict_el.is_terminate:
                        ...
                        dict
                            L list
                                ...
                                L list 2
                        -----
                        key:value(list)を扱うときは、value処理を行った後、dictの更新を行う
                        ------
                        ...
                        #list<list<dictなどの構成パターンは、外部で別途JsonDictで加工してから、追加・更新する
                        self.dictlist_elements[i].update_part(dict_el,hierarchy+1)
                ### end for
            ### end for
...
class DictList():
    ...
    def update_dict_by_dict(self,update_dict:dict,condition_dict:dict):
        ...
        for i in range(len(self.value)):
            dict_value:dict = self.value[i]
            if self.comp_dict_in_dict(dict_value,_condition_dict):
                dict_value.update(update_dict)
                self.value[i] = dict_value
